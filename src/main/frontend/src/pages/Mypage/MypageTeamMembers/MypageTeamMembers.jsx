import styles from "./MypageTeamMembers.module.css";
...
const MypageTeamMembers = () => {
  ...
  useEffect(() => {
    axios
      .get("/data/teamData.json")
      .then((response) => {
        const datas = response.data;
        const selectedTeam = datas.find(
          (data) => data.teamName === user.myTeam
        );
        if (selectedTeam) {
          ...
          // 유저가 팀에서 매니저인지 확인
          const userInTeam = selectedTeam.teamMember.find(
            (member) => member.name === user.nickname
          );
  ...
  const handleRoleChange = (index, newRole) => {
    ...
    // 알람창
    setNotification(
      `${teamState.teamMember[index].name}님의 직위가 ${newRole}로 변경되었습니다.`
    );
    setTimeout(() => {
  ...
  const handleButtonClick = () => {
    ...
    navigate({
      pathname: `/mypage/${user.id}/${teamState.teamCode}/application`,
    });
  };
...
export default MypageTeamMembers;
