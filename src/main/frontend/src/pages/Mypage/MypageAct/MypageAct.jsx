    ...
    <div style={{ gridArea: gridArea }}>
      <div className={styles.congrid}>
        <div className={styles.content}>
          ...
          {/* 예약한 경기 목록 */}
          <MatchesSection user={user} matches={upcomingMatches.slice(0, 2)} />
          {/* 지난 경기 기록 목록 */}
          <RecordsSection mathces={pastMatches.slice(0, 2)} id={user.id} />
          <TeamSection teamInfo={teamState} id={user.id} />
          <PostsSection contents={contents} comments={comments} commentCount={commentCount} user={user}/>
        </div>
      </div>
    </div>
  );
